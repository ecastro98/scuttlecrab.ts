...
export class Stop extends BaseInteractionCommandOption {
  constructor() {
    super({
      name: commandName,
      description: 'Stop the guild queue.',
      metadata: {
        description: 'Stop the guild queue.',
  ...
  async run(ctx: InteractionContext) {
    ...
    guild_player.destroy();

    await ctx.editOrRespond({
      content: `${Emojis.STOP} Stopped the music in this guild.`,
    });
  }
}
