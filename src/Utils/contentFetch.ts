...
import { MessageFinding } from './functions';
...
export async function fetchContent(base: string): Promise<LibData> {
  ...
  for (const module of modules) {
    for (const cl of module[1]) {
      ...
      for (const prop of cl[1])
        members.push({
          name: prop[0],
          path: cl[3],
          parentName: cl[0],
          type: SearchResultType.Property,
          prepared: fuzzy.prepare(prop[0])!,
          preparedWithParent: fuzzy.prepare(`${cl[0]}.${prop[0]}`),
          comment: prop[2],
        });
      for (const method of cl[2])
        members.push({
          name: method[0],
          path: cl[3],
          parentName: cl[0],
          type: SearchResultType.Method,
          prepared: fuzzy.prepare(method[0])!,
          preparedWithParent: fuzzy.prepare(`${cl[0]}.${method[0]}`),
          comment: method[2],
        });
    }
    for (const intf of module[2]) {
      ...
      for (const prop of intf[1])
        members.push({
          name: prop[0],
          path: intf[2],
          parentName: intf[0],
          type: SearchResultType.InterfaceProperty,
          prepared: fuzzy.prepare(prop[0])!,
          preparedWithParent: fuzzy.prepare(`${intf[0]}.${prop[0]}`),
        });
    }
    for (const en of module[3]) {
      ...
      for (const member of en[1])
        members.push({
          name: member[0],
          path: en[2],
          parentName: en[0],
          type: SearchResultType.EnumMember,
          prepared: fuzzy.prepare(member[0])!,
          preparedWithParent: fuzzy.prepare(`${en[0]}.${member[0]}`),
        });
    }
    for (const type of module[4])
      items.push({
        name: type[0],
        path: type[1],
        type: SearchResultType.Type,
        prepared: fuzzy.prepare(type[0])!,
      });
    for (const fn of module[5])
      items.push({
        name: fn[0],
        path: fn[1],
        type: SearchResultType.Function,
        prepared: fuzzy.prepare(fn[0])!,
      });
    for (const constant of module[6])
      items.push({
        name: constant[0],
        path: constant[1],
        type: SearchResultType.Constant,
        prepared: fuzzy.prepare(constant[0])!,
      });
  }
  return {
    items,
...
export function findInData(
  ...
  return res.map((result) => ({
    obj: result.obj,
    fullLink: `${settings.excludeBase ? '' : data.baseLink}/${result.obj.path
      .map((p) => `m.${data.moduleNames[p]}`)
      .join('/')}/${resultTypeToString[result.obj.type]}/${
      result.obj.parentName
        ? `${result.obj.parentName}.html#.${result.obj.name}`
        : `${result.obj.name}.html`
    }`,
    highlighted: settings.highlight
      ? fuzzy.highlight(result, settings.highlight, settings.highlight)
      : null,
  }));
}
